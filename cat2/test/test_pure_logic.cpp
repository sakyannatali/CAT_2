...
void test_model_inverse_for_250_lpm_at_supported_gates() {
  const int16_t gates[]={-100,-30,0,50,100};
  for(uint8_t i=0;i<sizeof(gates)/sizeof(gates[0]);++i) {
    const FlowModelSolveResult result=solveFlowModel(250.0f,gates[i]);
    ...
    TEST_ASSERT_FLOAT_WITHIN(0.05f,250.0f,estimate.totalLpm);
  }
}
...
void test_flow_setpoint_none_scale_and_pending_apply() {
  ...
  TEST_ASSERT_FLOAT_WITHIN(0.001f,35.0f,state.pendingLpm);
  flowSetpointInitialize(state,false,0.0f);
  for(uint8_t i=0;i<45;++i) flowSetpointAdjust(state,1,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM,3+i);
  TEST_ASSERT_TRUE(state.pendingDefined); TEST_ASSERT_FLOAT_WITHIN(0.001f,250.0f,state.pendingLpm);
  TEST_ASSERT_FALSE(flowSetpointAdjust(state,1,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM,50));
  TEST_ASSERT_FLOAT_WITHIN(0.001f,250.0f,state.pendingLpm);
  TEST_ASSERT_TRUE(flowSetpointCommit(state)); TEST_ASSERT_TRUE(state.appliedDefined); TEST_ASSERT_FLOAT_WITHIN(0.001f,250.0f,state.appliedLpm);
  for(uint8_t i=0;i<44;++i) flowSetpointAdjust(state,-1,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM,60+i);
  TEST_ASSERT_FLOAT_WITHIN(0.001f,30.0f,state.pendingLpm);
  TEST_ASSERT_TRUE(flowSetpointAdjust(state,-1,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM,110)); TEST_ASSERT_FALSE(state.pendingDefined);
  ...
  TEST_ASSERT_TRUE(flowSetpointValueAllowed(250.0f,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM));
  TEST_ASSERT_FALSE(flowSetpointValueAllowed(0.0f,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM));
  TEST_ASSERT_FALSE(flowSetpointValueAllowed(255.0f,FLOW_SETPOINT_MIN_LPM,FLOW_SETPOINT_MAX_LPM,FLOW_SETPOINT_EDIT_STEP_LPM));
...
void test_flow_setpoint_timeout_and_wraparound() {
  ...
  TEST_ASSERT_FALSE(state.editing); TEST_ASSERT_TRUE(state.pendingDefined); TEST_ASSERT_FLOAT_WITHIN(0.001f,250.0f,state.pendingLpm);
}
...
int main(int,char**) {
  ...
  RUN_TEST(test_model_inverse_and_status); RUN_TEST(test_model_inverse_for_250_lpm_at_supported_gates); RUN_TEST(test_model_status_from_actual_power);
