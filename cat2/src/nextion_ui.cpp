...
static UiField fields[]={{"fTIn",{}},{"fVolume1",{}},{"fVolume2",{}},{"fTSkin1",{}},{"fLSkin1",{}},{"fTSkin2",{}},{"fLSkin2",{}},{"fCompStatus",{}},{"fVentStatus",{}},{"fVentPower",{}},{"fGate",{}},{"fTimer",{}},{"fMode",{}},{"fSetpoint",{}},{"fPiOutput",{}},{"fError",{}}};
...
static void fieldValue(uint8_t index,char*out,size_t size,uint32_t now){char v[18];out[0]=0;switch(index){
  ...
  case 1: case 2:{const FlowState&s=app.flow[index-1];if(!s.valid)strcpy(out,"—");else if(s.calibrated){formatFixed1(s.filteredMassFlowKgH,v,sizeof(v));snprintf(out,size,"%s кг/ч",v);}else{formatFixed1(s.frequencyHz,v,sizeof(v));snprintf(out,size,"%s Hz",v);}break;}
  ...
  case 13:formatFixed1(app.flowSetpointKgH,v,sizeof(v));snprintf(out,size,"%s кг/ч",v);break;case 14:formatFixed1(app.piOutputPercent,v,sizeof(v));snprintf(out,size,"%s %%",v);break;
  default: if(!flowMeterIsCalibrated())strcpy(out,"FLOW NOT CALIBRATED");else if(app.autoBlocked){strncpy(out,app.autoBlockReason,size-1);out[size-1]=0;}else out[0]=0;break;
}}
static void endCommand(){NEXTION_SERIAL.write(0xFF);NEXTION_SERIAL.write(0xFF);NEXTION_SERIAL.write(0xFF);}
static void requestNumber(uint8_t which){pendingNumber=which;if(which==4)NEXTION_SERIAL.print(F("get sVentSpeed.val"));else if(which==5)NEXTION_SERIAL.print(F("get sGate.val"));else NEXTION_SERIAL.print(F("get nFlowSetpoint.val"));endCommand();}
static void handleTrigger(uint8_t id){switch(id){case 0:setCompressor(true);break;case 1:setCompressor(false);break;case 2:setVentEnabled(true);break;case 3:setVentEnabled(false);break;case 4:case 5:case 10:requestNumber(id);break;case 6:timerServiceStartOrPause(millis());break;case 7:timerServiceReset(millis());break;case 8:setFlowControlMode(FLOW_MANUAL);break;case 9:setFlowControlMode(FLOW_AUTO);break;}}
static void processInput(){while(NEXTION_SERIAL.available()){
  const uint8_t b=NEXTION_SERIAL.read();
  if(!packetUsed){if(b==0x23){packet[0]=b;packetUsed=1;packetExpected=0;}else if(b==0x71){packet[0]=b;packetUsed=1;packetExpected=8;}continue;}
  ...
  if(packetExpected&&packetUsed>=packetExpected){
    ...
    else if(packet[0]==0x71&&packetUsed==8&&packet[5]==0xFF&&packet[6]==0xFF&&packet[7]==0xFF){const uint32_t v=(uint32_t)packet[1]|((uint32_t)packet[2]<<8)|((uint32_t)packet[3]<<16)|((uint32_t)packet[4]<<24);if(pendingNumber==4)setFanPowerPercent(v>100?100:v);else if(pendingNumber==5)setGatePercent(v>100?100:v);else if(pendingNumber==10)setFlowSetpointKgH((float)v);pendingNumber=0;}
    packetUsed=packetExpected=0;
  }
}}
void nextionUiBegin(){NEXTION_SERIAL.begin(NEXTION_BAUD);}
void nextionUiService(uint32_t now){processInput();for(uint8_t n=0;n<3;++n){uint8_t i=cursor++%(sizeof(fields)/sizeof(fields[0]));char value[32];fieldValue(i,value,sizeof(value),now);sendText(fields[i],value);}}
