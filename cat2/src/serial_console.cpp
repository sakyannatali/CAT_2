...
static void help() {
  ...
  Serial.println(F("control manual|auto|setpoint NONE|30..500|status, debug off|sensors|all"));
}
...
static void printFlowSetpointEdit() {
  const FlowSetpointState &state=app.flowSetpoint;
  const uint32_t now=millis();
  Serial.print(F("Flow setpoint: applied="));
  if(state.appliedDefined) { Serial.print(state.appliedLpm,1); Serial.print(F(" L/min")); } else Serial.print(F("NONE"));
  Serial.print(F(" pending="));
  if(state.pendingDefined) { Serial.print(state.pendingLpm,1); Serial.print(F(" L/min")); } else Serial.print(F("NONE"));
  Serial.print(F(" editing=")); Serial.print(state.editing?F("yes"):F("no"));
  if(state.editing) { Serial.print(F(" age_ms=")); Serial.print(flowSetpointAgeMs(state,now)); Serial.print(F(" remaining_ms=")); Serial.print(flowSetpointRemainingMs(state,now,EDIT_APPLY_TIMEOUT_MS)); }
  Serial.println();
}
static void printModelSample(float power,float gate) {
  const FlowEstimate estimate=estimateFlow(power,(int16_t)gate);
  Serial.print(F("MODEL diagnostic only: P=")); Serial.print(power,1); Serial.print(F("% gate=")); Serial.print(gate,0); Serial.println(F("%"));
  Serial.print(F("  Q1/Q2/total: ")); Serial.print(estimate.output1Lpm,1); Serial.print('/'); Serial.print(estimate.output2Lpm,1); Serial.print('/'); Serial.print(estimate.totalLpm,1); Serial.println(F(" L/min"));
  Serial.print(F("  range status: ")); Serial.println(flowModelStatusText(flowModelStatusForPower(power,true)));
}
...
void serialConsolePrintStatus() {
  ...
  printFlowSetpointEdit();
  temperatureSensorsPrintStatus(); tofSensorsPrintStatus();
  char timer[16]; timerServiceFormat(timer,sizeof(timer));
  Serial.print(F("timer=")); Serial.println(timer);
  modelControlPrintStatus();
}

static void handle(char *s) {
  char *cmd=strtok(s," \t"), *arg=strtok(0," \t"), *arg2=strtok(0," \t"), *arg3=strtok(0," \t");
  if(!cmd) return;
  ...
  if(!strcmp(cmd,"flow")) {
    float power,gate;
    if(!arg || !strcmp(arg,"model")) {
      if(arg2 || arg3) {
        if(number(arg2,power)&&number(arg3,gate)&&power>=0&&power<=100&&gate>=GATE_COMMAND_MIN_PERCENT&&gate<=GATE_COMMAND_MAX_PERCENT) printModelSample(power,gate);
        else Serial.println(F("ERR: flow model <power 0..100> <gate -100..100>"));
      } else modelControlPrintStatus();
    } else help();
    return;
  }
  ...
  if(!strcmp(cmd,"control")) {
    ...
    else if(arg&&!strcmp(arg,"setpoint")&&number(arg2,v)) { if(!setFlowSetpointLpm(v))Serial.println(F("ERR: setpoint must be NONE or 30,35,...500 L/min")); }
...
void serialConsoleService() {
  ...
  if(debugLevel&&(uint32_t)(millis()-last)>=1000) { last=millis(); if(debugLevel>=1){temperatureSensorsPrintStatus();tofSensorsPrintStatus();} if(debugLevel>=2)modelControlPrintStatus(); }
}
